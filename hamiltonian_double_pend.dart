...
class AdvancedHamiltonianSolver {
  ...
  final double driveFreq = 1.5; // ड्रायव्हिंग फोर्सची वारंवारता

  /// Double Pendulum चे हॅमिल्टोनियन सूत्र: H = T + V
  double computeHamiltonian(DoublePhaseState state) {
    double dQ = state.q1 - state.q2;

    // मल्टिपल मॅट्रिक्स इनव्हर्जनचे सोपे केलेले कायनेटिक एनर्जी (T) चे समीकरण
  ...
  DoublePhaseState computeVectorField(DoublePhaseState state, double t) {
    ...
    // अंशिक प्रसरण (Partial Derivatives) - Numerical Automatic Differentiation
    double dH_dp1 =
        (computeHamiltonian(
...
/// ४. मुख्य डॅशबोर्ड विजेट (सिम्युलेशन आणि एक्सपोर्ट व्यवस्थापन)
...
class _ChaoticDashboardState extends State<ChaoticDashboard>
  ...
  DoublePhaseState _state = DoublePhaseState(
    ...
    0.0,
    0.0,
  ); // सुरुवातीचा कोन ९० अंश

  final List<FlSpot> _phaseHistory = [];
  final List<String> _csvRows = ["Time,q1,q2,p1,p2,Energy"]; // CSV डेटा बफर
  ...
  void initState() {
    ...
    _ticker = createTicker((Duration elapsed) {
      setState(() {
        // RK4 द्वारे नवीन स्टेट कॅल्क्युलेट करणे
        _state = _solver.rk4Step(_state, _dt, _currentTime);
        _currentTime += _dt;
        double currentEnergy = _solver.computeHamiltonian(_state);

        // १. चार्टसाठी डेटा ट्रॅक करणे (फक्त q1 आणि p1 चा आलेख दाखवत आहोत)
        _phaseHistory.add(FlSpot(_state.q1, _state.p1));
        if (_phaseHistory.length > 400) _phaseHistory.removeAt(0);

        // २. CSV साठी बॅकग्राउंडमध्ये डेटा गोळा करणे
        _csvRows.add(
  ...
  /// गोळा झालेला डेटा CSV फाईल म्हणून एक्सपोर्ट आणि शेअर करणे
  Future<void> _exportToCSV() async {
    setState(() => _isExporting = true);
    try {
      // १. सर्व रो एकत्र करून एका स्ट्रिंगमध्ये बदलणे
      final String csvContent = _csvRows.join("\n");

      // २. पाथ आणि फाईल सिस्टीम ऐवजी थेट मेमरी डेटा वापरणे (नो पाथ एरर)
      final XFile csvFile = XFile.fromData(
        Theme.of(context).platform == TargetPlatform.android ||
                Theme.of(context).platform == TargetPlatform.iOS
            ? const Utf8Encoder().convert(csvContent) // मोबाईलसाठी बाईट्स रूप
            : const Utf8Encoder().convert(csvContent),
        mimeType: 'text/csv',
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Export Error: ${e.toString()}")));
    } finally {
...
/// ५. २D ग्राफिक्स इंजिन (Double Pendulum Custom Painter)
class DoublePendulumPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const double scale = 80.0; // Scale from meters to pixels
    ...
    // पहिल्या लंबकाचे कार्टेशियन कोऑर्डिनेट्स (x1, y1)
    ...
    // दुसऱ्या लंबकाचे कार्टेशियन कोऑर्डिनेट्स (x2, y2)
